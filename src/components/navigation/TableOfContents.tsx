...
export default function TableOfContents() {
  ...
  return (
    <nav className="flex flex-col gap-1.5 text-sm max-w-full">
      {toc.map(({ id, text, level }) => (
        <a
          ...
          className={cn(
            'group flex items-center gap-3 text-stone-600 hover:text-stone-800 transition-all',
        ...
        >
          <span
            className={cn(
              'h-px bg-stone-500 transition-all shrink-0',
              level === 3 ? 'w-8' : 'w-6',
              'origin-left transition-all',
              'group-hover:h-0.5',
              'group-data-[active=true]:bg-yellow-600 group-data-[active=true]:h-0.5',
            )}
          />
          {text}
        </a>
